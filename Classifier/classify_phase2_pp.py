...
for sbj in subjects: # because you're inputting a number it wants to parse it if you use a for loop
	...
	p2_volinfo = expdir+f'/volinfo/phase2/sub-{sbj}/{sbj}_volinfo_3TR.txt'
	stim_types,TRs = loadtxt(p2_volinfo,unpack=1)

	subjdir = expdir+'/sub-%s'%(sbj)

	maskdir = expdir + f'/cluster_maps/sub-{sbj}/transformed/dilD_345/'

	boldir = expdir + f'/sub-{sbj}/smoothed_phase2/'
	...
	for mask in masks:
		...
		locfuncs = [boldir + f'/sub-{sbj}_task-ChoiceCB_2mm_hpf32.nii.gz']


		locds = fmri_dataset(locfuncs,mask=classmask,targets=stim_types)

		#poly_detrend(locds,polyord=1,chunks_attr='chunks')
		locds = locds[locds.targets != 0] # drop empty points
		zscore(locds) # not sure if it will work
		...
		phase,pp_runs,scene,_ = loadtxt(pp_volinfo,unpack=1)

		betadir = subjdir + "/merged_betaseries/"
		prepostfuncs = [betadir + f'/sub-{sbj}_prepost_merged_betaseries.nii.gz']

		pp_ds = fmri_dataset(prepostfuncs, chunks=pp_runs, mask=classmask, targets=scene)
		...
		clf.train(post_ds)
		print('trained!')
		pre_pred = clf.predict(locds)
		prefile = "results//%s_label_betas_pre_%s.txt"%(sbj,mask.split('_mask_transformed_dilD_345.nii.gz')[0])
