...
class SeedConnectivity():
    def __init__(self,sub_id:int=5159,bold_path:str='../../R01-fmri/fmriprep_pepolar/',seed_region:str="Hippocampus"):
        ...
        self.output_dir = Path.cwd() / "results" / "plot_seed_to_voxel_correlation_rumi" #put the output files in my rumi folder
    ...
    def get_subco_seed(self,coords:tuple=(9,19),bilateral:bool=True):
        ...
        seed_masker = NiftiLabelsMasker( #SEED_MASKER LIKE BEFORE
            labels_img=l_mask,
            detrend=True,
            standardize=True,
            standardize_confounds=True,
    ...
    def plot_seed_voxel_map(self,brain_masker,seed_to_voxel_correlations):
        ...
        display = plotting.plot_stat_map(
            ...
            title="Seed-to-voxel correlation (PCC seed)", #NOT PCC
        )
        display.add_markers(
    ...
    def get_ROI_to_ROI_fisher_z(self,seed_time_series,brain_time_series): #IGNORE THIS FUNCTION
    ...
    def get_seed_connectivity_nifti(self,func_file=None,confound_file=None, partition=None):
        ...
        brain_masker = self.mask_timeseries(region="gm") #changed from OFC to gm
        brain_time_series = self.fit_timeseries(brain_masker,func_file,confound_file)

        self.get_ROI_to_ROI_fisher_z(seed_time_series=seed_time_series, brain_time_series=brain_time_series) #IGNORE THIS FUNCTION, ONLY SAVE_FISHER_NIFTI WAS USED
...
if __name__ == "__main__":
    sub_id=sys.argv[1] #indexes the subject number when u run something like python3 seed_connectivity.py 700
